...
public interface ManagerRepository extends JpaRepository<Manager, Integer> {
	...
	//La media y desviación estándar del número de notas escritas por manager
	@Query("select avg(c.annotationWriter.size) from Manager c")
	Object[] avgDesviationNotesWrittersByManagers();

	//La media y desviación estándar del número de notas asociadas a managers
	@Query("select avg(c.annotationStore.size) from Manager c")
	Object[] avgDesviationNotesStoreByManagers();
